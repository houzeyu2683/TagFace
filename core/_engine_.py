...
class Action:
    ...
    def saveSegment(self) -> bool:
        ...
        with open(os.path.join(checkpoint, 'segment.json'), 'w') as paper:
            ...
            pass
        _ = paper
        os.makedirs(os.path.join(checkpoint, 'video'), exist_ok=True)
        segment = self.segment
        for index, (interval, region) in segment.items():
            path = os.path.join(checkpoint, 'video', f'{index}.mp4')
...
# engine = Engine(folder='./episode/PL9mUJWHev0Klo61lR1HwHxSvngSnhKAQg')
# engine.processBatch(channel='./download/PL9mUJWHev0Klo61lR1HwHxSvngSnhKAQg/video', core=4)
